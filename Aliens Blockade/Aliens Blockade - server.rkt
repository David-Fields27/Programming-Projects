...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname final-server) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
...
(define INIT-LOA (build-list NUM-ALIENS (λ (n) (make-posn (+ STARTING-X-LINE (remainder n ALIENS-PER-LINE))
                                                          (* 3 (quotient n ALIENS-PER-LINE))))))
(define INIT-LOS '())

;; Contants for INIT-WORLD2 
(define INIT-ROCKET2 (make-posn 15 MAX-IMG-Y))
(define INIT-MOVETIMER2 3)
(define INIT-ALIEN2 (make-posn 3 MAX-IMG-Y))
(define DIR2 'left)
(define SHOT2     (make-posn AN-IMG-X (/ (sub1 MAX-CHARS-VERTICAL) 2)))

;; An ally is a structure: (make-ally rocket string mt)
;; with the player's rocket, name, and move timer
;; A rocket is either:
;; 1. 'finished-ally
;; 2. a posn: (make-posn x y)
;; A mt is a natum>0
(define-struct ally (rocket name mt))
...
;; Sample instances of ally
(define INIT-ALLY  (make-ally INIT-ROCKET  MY-NAME INIT-MOVETIMER))
(define INIT-ALLY2 (make-ally INIT-ROCKET2 MY-NAME INIT-MOVETIMER2))
...
(define WORLD3 (make-world (list (make-ally (make-posn 7 MAX-IMG-Y) "iworld1" 2)
                                 (make-ally (make-posn 9 4) "iworld2" 1))
                           (list (make-posn 3 3))
                           'right
                           (list (make-posn 1 2))))
(define WORLD4 (make-world (list (make-ally FINISHED-ALLY "iworld3" 4)
                                 (make-ally (make-posn 5 1) "iworld2" 1))
                           (list (make-posn 8 2))
                           'right
                           (list (make-posn 8 4))))
...
(define (marshal-ally an-ally)
  (if (posn? (ally-rocket an-ally))
      (list (posn-x (ally-rocket an-ally)) (posn-y (ally-rocket an-ally))
            (ally-name an-ally) (ally-mt an-ally))
      (list (ally-rocket an-ally) (ally-name an-ally) (ally-mt an-ally))))

;; Sample expressions for marshal-ally 
(define M-ALLY1 (list (posn-x (ally-rocket INIT-ALLY)) (posn-y (ally-rocket INIT-ALLY))
                      (ally-name INIT-ALLY) (ally-mt INIT-ALLY)))
(define M-ALLY2 (list (posn-x (ally-rocket INIT-ALLY2)) (posn-y (ally-rocket INIT-ALLY2))
                      (ally-name INIT-ALLY2) (ally-mt INIT-ALLY2)))
...
(check-expect (marshal-ally  (make-ally (make-posn 12 14) "Cordula" 3))
              (list 12 14 "Cordula" 3))
(check-expect (marshal-ally  (make-ally FINISHED-ALLY "Renna" 3))
              (list 'finished-ally "Renna" 3))
...
(check-expect (marshal-world (make-world (list (make-ally (make-posn 5 5) "Luis" 3)
                                               (make-ally (make-posn 8 8) "Cova" 3))
                                         (list (make-posn  2 17))
                                         "left"
                                         '()))
              (list (list (list 5 5 "Luis" 3)
                          (list 8 8 "Cova" 3))
                    (list (list  2 17))
                    "left"
                    '()))
...
(check-expect (marshal-world (make-world (list (make-ally (make-posn 5 5) "Luis" 3)
                                               (make-ally FINISHED-ALLY "Cova" 3))
                                         (list (make-posn  2 17))
                                         "left"
                                         '()))
              (list (list (list 5 5 "Luis" 3)
                          (list 'finished-ally "Cova" 3))
                    (list (list  2 17))
                    "left"
                    '()))
...
(define (process-tick a-univ)
  (local [;; dir --> (alien --> alien)
          ...
          (define (move-alien-maker a-dir)
            (lambda (an-alien)
              (local [;; image-x>min \arrow image-x
          ...
          ;; lor --> lor
          ;; Purpose:Determines whether to move the given listof allies up,
          ;;         move a given ally down if they crashed, or turn a given ally into a finished-ally
          (define (move-allies a-lor a-loa)
            (local [;; lor --> boolean
                    ;; Purpose: Determine if an ally crashes into an another ally
                    (define (ally-ally-crash? an-ally a-lor)
                     (ormap (λ (an-ally2)
                                  (and (posn? (ally-rocket an-ally2))
                                       (= (posn-y (ally-rocket an-ally2)) (posn-y (ally-rocket an-ally)))
                                       (= (posn-x (ally-rocket an-ally2)) (posn-x (ally-rocket an-ally)))
                                       (not (eq? (ally-name an-ally2) (ally-name an-ally)))))
                          a-lor))
                    ;; lor --> boolean
                    ;; Purpose: Determine if an ally crashes into an alien
                    (define (alien-ally-crash? an-ally a-loa)
                     (ormap (λ (an-alien)
                              (and (= (posn-y an-alien) (posn-y (ally-rocket an-ally)))
                                   (= (posn-x an-alien) (posn-x (ally-rocket an-ally)))))
                          a-loa))]
            (map (λ (an-ally)
                   (cond [(eq? (ally-rocket an-ally) FINISHED-ALLY) an-ally]
                         [(and (posn? (ally-rocket an-ally)) (ally-ally-crash? an-ally a-lor))
                           (make-ally
                              (make-posn (posn-x (ally-rocket an-ally))
                                         MAX-IMG-Y)
                              (ally-name an-ally)
                              INIT-MOVETIMER)]
                         [(and (posn? (ally-rocket an-ally)) (alien-ally-crash? an-ally a-loa))
                           (make-ally
                              (make-posn (posn-x (ally-rocket an-ally))
                                         MAX-IMG-Y)
                              (ally-name an-ally)
                              INIT-MOVETIMER)]
                         [(= (posn-y (ally-rocket an-ally)) MIN-IMG-Y)
                          (make-ally FINISHED-ALLY
                                     (ally-name an-ally)
                                     (ally-mt an-ally))]
                         [else (if (= (ally-mt an-ally) 0)
                                   (make-ally
                                    (make-posn (posn-x (ally-rocket an-ally))
                                         (sub1 (posn-y (ally-rocket an-ally))))
                                    (ally-name an-ally)
                                    INIT-MOVETIMER)
                                   (make-ally
                                    (make-posn (posn-x (ally-rocket an-ally))
                                         (posn-y (ally-rocket an-ally)))
                                    (ally-name an-ally)
                                    (sub1 (ally-mt an-ally))))]))
                 a-lor)))
          ...
          (define (new-dir-after-tick a-loa old-dir)
            (local [;; loa --> Boolean
              ...
              (if (eq? old-dir 'left)
                  ...
                  (if (any-alien-at-right-edge? a-loa) 'left 'right))))
          ...
          (define (process-tick a-world)
            (make-world (move-allies (world-allies a-world) (world-aliens a-world)) 
...
(check-expect (process-tick
               (make-univ (list iworld1 iworld3)
                          (make-world (list (make-ally (make-posn 9 MAX-IMG-Y) "iworld1" 3)
                                            (make-ally (make-posn 2 MAX-IMG-Y) "iworld3" 3))
                                      (list (make-posn 2 5))
                                      'left
                                      (list (make-posn 3 6) NO-SHOT))))
              (make-bundle
               (make-univ (list iworld1 iworld3)
                          (make-world (list (make-ally (make-posn 9 14) "iworld1" 2)
                                            (make-ally (make-posn 2 14) "iworld3" 2))
                                      (list (make-posn 1 5))
                                      'left
                                      (list (make-posn 3 5))))
               (list (make-mail iworld1
                                (list 'world
                                      (list (list 9 14 "iworld1" 2)
                                            (list 2 14 "iworld3" 2))
                                      (list (list 1 5))
                                      'left
                                      (list (list 3 5))))
...
(check-expect (process-tick
               (make-univ (list iworld3)
                          (make-world (list (make-ally (make-posn 6 1) "iworld3" 2))
              ...
              (make-bundle
               (make-univ
                (list iworld3)
                (make-world (list (make-ally (make-posn 6 1) "iworld3" 1))
                            (list (make-posn 17 10))
                            'left
...
(check-expect (process-tick
               (make-univ
                (list iworld2)
                (make-world (list (make-ally (make-posn 14 0) "iworld2" 0))
              ...
              (make-bundle
               (make-univ
                (list iworld2)
                (make-world
                 (list (make-ally FINISHED-ALLY "iworld2" 0))
                 (list (make-posn 20 2))
                 'right
                 (list (make-posn 15 5))))
               (list (make-mail
                      iworld2
                      (list 'world
                            (list (list 'finished-ally "iworld2" 0))
                            (list (list 20 2))
                            'right
                            (list (list 15 5)))))
               '()))
...
(check-expect (process-tick
               (make-univ
                (list iworld1)
                (make-world (list (make-ally (make-posn 3 4) "iworld1" 4))
              ...
              (make-bundle
               (make-univ
                (list iworld1)
                (make-world
                 (list (make-ally  (make-posn 3 4) "iworld1" 3))
                 (list (make-posn 1 2))
                 'right
                 '()))
               (list (make-mail
          ...
          (define (process-key a-key)
            (local [;; shot rocket --> shot
                    ...
                    (define (make-ally-mover move-rckt)
                      (λ (a-name a-lor)
                        (map (λ (an-ally)
                               (if (and (posn? (ally-rocket an-ally)) (string=? a-name (ally-name an-ally)))
                                   (make-ally (make-posn (move-rckt (posn-x (ally-rocket an-ally)))
                                                         (posn-y (ally-rocket an-ally)))
                                              (ally-name an-ally)
                                              (ally-mt an-ally))
                                   an-ally))
...
(check-expect
 ...
 (make-bundle
  (make-univ
   ...
   (make-world
    (list (make-ally (make-posn 7 14) "iworld1" 2) (make-ally (make-posn 8 4) "iworld2" 1))
    (list (make-posn 3 3))
    'right
    (list (make-posn 1 2))))
  (list
   (make-mail
    ...
    (list
     'world
     (list (list 7 14 "iworld1" 2) (list 8 4 "iworld2" 1))
     (list (list 3 3))
     'right
     (list (list 1 2))))
   (make-mail
...
(check-expect
 ...
 (make-bundle
  (make-univ
   (list iworld1 iworld2)
   (make-world
    (list (make-ally (make-posn 8 14) "iworld1" 2) (make-ally (make-posn 9 4) "iworld2" 1))
    (list (make-posn 3 3))
    'right
    (list (make-posn 1 2))))
  (list
   (make-mail
    ...
    (list
     'world
     (list (list 8 14 "iworld1" 2) (list 9 4 "iworld2" 1))
     (list (list 3 3))
     'right
     (list (list 1 2))))
   (make-mail
...
(check-expect
 ...
 (make-bundle
  (make-univ
   (list iworld1 iworld2)
   (make-world
    (list (make-ally (make-posn 7 14) "iworld1" 2) (make-ally (make-posn 9 4) "iworld2" 1))
    (list (make-posn 3 3))
    'right
  ...
  (list
   (make-mail
    ...
    (list
     'world
     (list (list 7 14 "iworld1" 2) (list 9 4 "iworld2" 1))
     (list (list 3 3))
     'right
...
(check-expect
 (process-message OTHR-UNIV2 iworld2 SHOOT)
 (make-bundle
  (make-univ
   (list iworld3 iworld2)
   (make-world
    (list (make-ally FINISHED-ALLY "iworld3" 4) (make-ally (make-posn 5 1) "iworld2" 1))
    (list (make-posn 8 2))
    'right
    (list (make-posn 5 1) (make-posn 8 4 ))))
  (list
   (make-mail
    iworld3
    (list
     'world
     (list (list 'finished-ally "iworld3" 4) (list 5 1 "iworld2" 1))
     (list (list 8 2))
     'right
     (list (list 5 1) (list 8 4))))
   (make-mail
    iworld2
    (list
     'world
  ...
  (if (member? (iworld-name an-iw) (map iworld-name (univ-iws a-univ)))
      ...
      (local [(define new-iws (cons an-iw (univ-iws a-univ)))
              ...
              (define new-game (if (equal? game UNINIT-WORLD)
                                   (make-world
                                    (list (make-ally INIT-ROCKET (iworld-name an-iw) INIT-MOVETIMER))
                                    (world-aliens INIT-WORLD)
                                    (world-dir    INIT-WORLD)
                                    (world-shots  INIT-WORLD))
                                   (make-world
                                    (cons (make-ally INIT-ROCKET (iworld-name an-iw) INIT-MOVETIMER)
                                          (world-allies game))
                                    (world-aliens game)
                                    (world-dir game)
                                    (world-shots game))))]
...
(define EMP-ADD (local [(define new-iws (cons iworld2 (univ-iws INIT-UNIV)))
                        ...
                        (define new-game (if (equal? game UNINIT-WORLD)
                                             (make-world
                                              (list (make-ally INIT-ROCKET (iworld-name iworld2) 3))
                                              INIT-LOA
                                              INIT-DIR
                                              INIT-LOS)
                                             (make-world
                                              (cons (make-ally INIT-ROCKET (iworld-name iworld2) 3)
                                                    (world-allies game))
                                              (world-aliens game)
                                              (world-dir game)
                                              (world-shots game))))]
...
(define NEW-ADD (local [(define new-iws (cons iworld3 (univ-iws OTHR-UNIV)))
                        ...
                        (define new-game (if (equal? game UNINIT-WORLD)
                                             (make-world
                                              (list (make-ally INIT-ROCKET (iworld-name iworld3) 3))
                                              INIT-LOA
                                              INIT-DIR
                                              INIT-LOS)
                                             (make-world
                                              (cons (make-ally INIT-ROCKET (iworld-name iworld3) 3)
                                                    (world-allies game))
                                              (world-aliens game)
                                              (world-dir game)
                                              (world-shots game))))]
...
(check-expect (add-player (make-univ (list iworld2 iworld3)
                                     (make-world
                                      (list (make-ally (make-posn 7 4) "iworld2" 2) (make-ally (make-posn 9 3) "iworld3" 2))
                                      (list (make-posn 3 3))
                                      'right
                                      (list (make-posn 3 3))))
                          iworld1)
              (make-bundle   (make-univ
                              ...
                              (make-world
                               (list (make-ally (make-posn 10 14) "iworld1" 3)
                                     (make-ally (make-posn 7 4) "iworld2" 2)
                                     (make-ally (make-posn 9 3) "iworld3" 2))
                               (list (make-posn 3 3))
                               'right
                               (list (make-posn 3 3))))
                             (map
...
(check-expect (add-player (make-univ (list iworld2 iworld3)
                                     (make-world
                                      (list (make-ally FINISHED-ALLY "iworld2" 2) (make-ally (make-posn 9 3) "iworld3" 2))
                                      (list (make-posn 3 3))
                                      'right
                                      (list (make-posn 3 3))))
                          iworld1)
              (make-bundle   (make-univ
                              ...
                              (make-world
                               (list (make-ally (make-posn 10 14) "iworld1" 3)
                                     (make-ally FINISHED-ALLY "iworld2" 2)
                                     (make-ally (make-posn 9 3) "iworld3" 2))
                               (list (make-posn 3 3))
                               'right
                               (list (make-posn 3 3))))
...
(check-expect (rm-player (make-univ
                          (list iworld3)
                          (make-world (list (make-ally (make-posn 8 14) "iworld3" 1))
                                      '()
                                      'down
                                      '()))
                         iworld3)
